  ...
  useEffect(() => {
    moveFromTo.map((data) => {
      if (data.from === head && data.to !== head) {
        ...
        if (ind > -1) tempCards.splice(ind, 1);
        setCards(tempCards);
    ...
    <div className="list">
      {title === 1 ? (
        <div className="listTitle">
          ...
          ></input>
          <div className="listHead" onClick={addTitle}>
            +
          </div>
        </div>
      ) : (
      ...
      {edit !== -1 ? (
        <EditList data={cards[edit]} onSubmit={editChanges} index={edit} />
      ) : (
        <div>
          ...
          <div className="listAddCard" onClick={addCard}>
            {" "}
            + Add another card{" "}
          </div>
        </div>
